...
- ✅ **PCI-DSS alignment** via WAF (OWASP rules), private endpoints, network segmentation
- ✅ **Azure AD-only authentication** for SQL Database (Microsoft-recommended, eliminates SQL auth attack vector)
- ✅ **Managed identities** for all service-to-service communication (no secrets in code)
- ✅ **Private endpoints** for SQL, Redis, Cognitive Search, Service Bus, Key Vault
- ✅ **Azure Front Door WAF** with OWASP Core Rule Set for edge protection
- ✅ **Network segmentation** with 3-tier subnet isolation (web, data, integration)
- ✅ **NSG deny-by-default** at priority 4096
...
- ✅ **Zone redundancy** with App Service P1v3 (minimum 2 instances across availability zones)
- ✅ **99.9% SLA achievable** with Premium SKU and multi-instance configuration
- ✅ **Service Bus Premium** with private endpoints for reliable async processing
- ✅ **Azure Functions EP1** with VNet integration for event-driven scaling
- ✅ **Redis Standard C2** for session caching (handles 10K concurrent users)

**Gap Analysis:**

- ⚠️ **Single-region deployment** - no automatic DR failover configured
- ⚠️ **SQL geo-replication** not included in baseline design
- ⚠️ **RPO/RTO not defined** for disaster recovery scenarios
...
- ✅ **Azure Cognitive Search S1** achieves sub-100ms catalog queries with faceted navigation
- ✅ **Redis C2 (6GB)** handles 10K concurrent sessions with low latency
- ✅ **Azure Front Door CDN** delivers static assets from edge locations globally
- ✅ **App Service P1v3** provides dedicated compute with autoscale capability
- ✅ **Static Web Apps** with global edge distribution for React SPA

**Capacity Validation:**

| Component | Requirement | Proposed SKU | Capacity |
|-----------|-------------|--------------|----------|
| Concurrent Users | 10,000 | App Service P1v3 x2 | ✅ Adequate (can scale to 12 instances) |
| Catalog Queries | <100ms | Cognitive Search S1 | ✅ Adequate (15M docs, 35GB vector index) |
| Session Cache | 10K sessions | Redis C2 (6GB) | ✅ Adequate (~600 bytes/session avg) |
| Async Processing | Order queue | Service Bus Premium | ✅ 1M messages/day supported |
...
- ✅ **Log Analytics** with 90-day retention for compliance logging
- ✅ **Deployment slots** for zero-downtime deployments
- ✅ **Azure Functions** with runtime scale monitoring enabled
- ✅ **Infrastructure as Code** approach (Bicep templates recommended)
...
3. **Search Index Strategy**: How should SQL → Cognitive Search sync work?
